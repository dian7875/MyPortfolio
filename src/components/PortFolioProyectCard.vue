<template>
   ...
   border-zinc-300
   dark:border-zinc-700 flex gap-4 max-lg:flex-col">
        <div class=" w-1/3 max-lg:w-full">
            <img v-if="srcImg" :src="currentImg" :alt="`Vista previa del proyecto ${name}`"
                class="w-full h-[15rem] transition-opacity duration-500 ease-in-out opacity-100"
                @mouseover="changeImage(true)" @mouseout="changeImage(false)">
        </div>
        <div class=" w-2/3 space-y-1 max-lg:w-full">
            <h3 class="text-2xl font-bold  max-md:text-lg">
                {{ name }}
            </h3>
            <h4 class="text-lg font-bold  max-md:text-base">
                {{ role }}
            </h4>
            <p class=" text-sm">
                {{ resumen }}
